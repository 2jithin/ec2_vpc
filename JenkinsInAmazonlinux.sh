...
INSTANCE_ID=$(aws ec2 run-instances --image-id ami-0cc87e5027adcdca8 --count 1 --instance-type t2.micro --security-group-ids $SG_ID --subnet-id $SUBNET_ID --associate-public-ip-address --tag-specifications 'ResourceType=instance,Tags=[{Key=jenkinsserver,Value=Test}]' 'ResourceType=volume,Tags=[{Key=jenkinsserver, Value=Test}]' --user-data "#!/bin/bash
sudo yum update -y
